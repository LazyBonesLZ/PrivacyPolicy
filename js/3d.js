    ...
    $.fn.lucky = function (opt) {
                ...
                for (var r = 0; r < settings.col; r++) {
                    if (i == M.centerX && r == M.centerY) {
                        if (!M.isThisTag) {
                            ...
                            eleStr += '<div style="' + styleFun(i, r) + '" class="js_current_dom"><img class="lucky_icon" /><p class="_userName">网吧</p></div>';
                            M.isThisTag = true;

                        }

                    } else {
                        // eleStr += '<div data-depth="' + n + '" style="' + styleFun(i, r) + '" class="element"><img class="lucky_icon" /><p class="_userName">娃娃</p></div>';
        ...
        var stepFun = function () {
            ...
            for (var i = 0; i < elements; i++) {
                ...
                if (!!settings.data[i]) {
                    ...
                    // $("._userName").text(settings.data[i].name)
                    //  $("._userName").attr('class', '_userName').attr('style', '');
                    // $(".lucky_icon").attr('src', settings.data[i].image)
                } else {
                    index >= (settings.data.length - 1) ? index = 1 : ++index;
                    $(element).css('background-image', 'url(' + settings.data[index].image + ')');
                     // $(".lucky_icon").attr('src', settings.data[index].image)
                     // $("._userName").text(settings.data[index].name)
                     //  $("._userName").attr('class', '_userName').attr('style', '');
                }
        ...
        var randomFun = function () {
            ...
            M.timer = setInterval(function () {
                ...
                $('.js_current_dom').children($("p")).text(settings.data[randomNum].name)
