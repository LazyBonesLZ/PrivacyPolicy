$(function () {
    ...
    var lotteryTypes = ["The 3rd Prize","The 2nd Prize","Lucky Prize","The 1st Prize"]
    var lotteryCount = [15,6,10,1]
    ...
    初始化参与人数
     */
      $('.lucky_number').html(currentLottoryCount)
    /*
     图片预加载
     */
...
// 一等奖要去掉我和kimmy  桑心
    function excludeWhen1stPrize(index){
        if (currentLottoryType == "The 1st Prize") {
            var people = personArray[index]
            if (people.id == "AE1877" || people.id == "NX0017") {
                return true;
            };
        };
        return false;
    ...
    function randomLuckyArr() {
        ...
        for (var i = 0; i < Obj.luckyNum; i++) {
            ...
            var bExclude = excludeWhen1stPrize(random)
            if (Obj.luckyResult.indexOf(random) == -1 && !bExclude) {
    ...
    function tabSport(i, lucky_prize_number) {
        ...
        $('.lucky_number').html(currentLottoryCount)
        //set default value
        $(".select_lucky_number").val(currentLottoryCount)
        Obj.luckyNum = currentLottoryCount
